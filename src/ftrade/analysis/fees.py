...
def fee_summary(
    fees: pd.DataFrame, fx: FX, start: str | None = None, end: str | None = None
) -> dict:
    """Total fees paid, converted into the base currency and broken down.

    ``start``/``end`` filter by ``create_time`` -- a fee is realised the day
    the order that incurred it happened, not tied to any round trip.
    """
    ...
    df = fees.copy()
    if start or end:
        df = df[_in_range(df.get("create_time", pd.Series(dtype=str)).astype(str), start, end)]
    if df.empty:
        return {"total": 0.0, "count": 0, "base_currency": fx.base}
    ...
    df["month"] = df.get("create_time", pd.Series(dtype=str)).astype(str).str[:7]

    by_year = df.groupby("year")["base"].sum().sort_index()
    by_month = df.groupby("month")["base"].sum().sort_index()
    by_symbol = (
    ...
    return {
        ...
        "by_month": {str(k): round(float(v), 2) for k, v in by_month.items() if k},
        "by_currency": {
